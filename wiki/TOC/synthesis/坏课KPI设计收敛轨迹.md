...
- **问题账条目**：TOC-15（坏课迟到阈值未冻结）、TOC-16（「假早退」判定未入设计稿）、TOC-17（坏课 KPI 数据源体系张力）、TOC-18（坏课基线数字未核验）、TOC-30（出营率与坏客率两尺并行）、TOC-67（坏课率过渡组合未拍死）、TOC-69（网络/设备坏课判定标准待建立）——本页「红线/阈值未冻结、先冻结基线再挂目标」的收敛状态与上述待办同源；外援页未涉及迟到阈值细节与假早退口径，见编译报告「矛盾与盲点」。
