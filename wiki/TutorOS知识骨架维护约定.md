...
## 现行维护边界（2026-09-24 核对）

2026-09-23 的[骨架问题闭环合同](../_ops/frontier-loop/contract.md)（本地资料，钉钉／GitHub未提供）已增加骨架改稿前的人审闸门：骨架正文、相关链接、DELTA、H/Q、NOW/MODEL 的任何新增改写，先形成具体候选差异，获 Leon 对该差异认可后再写正本。下层正文、落点登记和派生导航仍可在维护任务授权内按来源规则处理。本页较早的日常维护描述按此范围理解；不把一次“优化”或待审标签当作具体差异认可。
...
| 下层页面、落点登记与派生导航的日常维护 | Agent | 在任务授权和现有来源规则内维护；涉及骨架正文及链接时先提交具体差异人审 |
...
<a id="经营域页面的共同写法2026-09-15"></a>
## 经营域页面的共同写法（2026-09-17）

按[经营域共建模板](经营域共建模板.md) （[A](https://alidocs.dingtalk.com/i/nodes/nYMoO1rWxaZnB3GdI9kzrodPV47Z3je9) · [Leon](https://alidocs.dingtalk.com/i/nodes/pYLaezmVNejqr3vQtKaGqG6ZWrMqPxX6)）统一为六模块：**NOW｜现在怎么看、MODEL｜价值怎样产生、BELIEFS｜哪些判断仍待检验、FRONTIER｜接下来想弄清什么、DELTA｜认识怎样走到这里、BOUNDARIES｜边界与下钻**。名称、顺序与 D03、D05 一致；各域用自身问题、机制、事实和取舍填充，不能只替换标题。

NOW 用核心问题、当前判断、当前重点接上思考；客户价值和重要取舍进入 MODEL。年度和季度重点仅在有依据、确能帮助判断时保留并标时点，不再强制每域补齐两栏；AI 建议不能写成公司已经确定的重点。

H 编号保留有条件的工作假设、依据、反证与改判条件；Q 编号保留未想明白的问题，必要时连接 H，数量按真实内容决定。DELTA 只记录有来源的认识变化、决定或纠错，排版和同步放维护记录。BOUNDARIES 说明本域与相邻域、决策、执行记录的分工。

**第一、第二层给 Leon 看的正文全部使用非技术语言。** 先说现在怎么看，再用具体人、事和结果解释；保留会影响判断的条件、代价和相反事实。完整证据登记、来源阅读范围、历史与相关知识目录放在各域 `Dxx-认知依据与演化` 附页，主文 E 编号直达。

2026-09-15 的七项写法保留在历史认领记录中；2026-09-17 Leon 确认六模块模板，并明确要求其余骨架页按 D03/D05 同一标准改造，由 subagent 成稿、主 agent 检查和验收。模板认可、工程验收、逐条人审与业务效果分别记录，不能互相替代。

<a id="当下的好奇怎样参与蒸馏"></a>
## “FRONTIER｜接下来想弄清什么”怎样参与蒸馏
...
1. **先读清材料，再回看问题。** 读取本次相关经营域的“FRONTIER｜接下来想弄清什么”；看看材料是否推进旧问题、带来另一种解释或相反例子，或者提出地图里还没有的新问题。已列问题只是帮助留意线索，不能用它筛掉其他有价值的内容。
2. **有推进才提出改稿。** 在对应问题的候选差异中简短记下日期、原材料说了什么、让我们多明白了什么、还有哪里没明白，并链接出处；按现行合同获具体差异认可后再更新骨架。只出现相同词语、重复旧观点不算推进；材料没有提到某事，也不算否定它。没有进展时不改日期、不填流水。
3. **按依据的分量保留答案。** 单个案例先作为线索，部分回答保留未解之处。开始有暂时答案时可进入 BELIEFS；有更充分依据时更新 NOW、MODEL 或相应 H/Q，再在 DELTA 留下变化。同一来源的不同整理稿不能算多份证据。
4. **让问题继续变化。** 可以改问法、合并重复问题、加入新问题。跨域问题留在最能解释它的经营域，其他域引用；暂时没有合适位置时沿现有原页或 query 留下，不强行归类。退出当前清单时保留阶段答案或放下原因，以及原讨论入口。
5. **把有分量的变化带回来。** 普通线索与下层页面整理由 AI 维护；骨架页面按现行人审闸门处理；足以改变核心看法的发现，带着原问题、新证据、具体改稿和影响回到 Leon 所在的骨架层。普通好奇留在域内；只有形成需要验证的重要假设、需要裁决的冲突或选择时，才按原有规则关联问题账，避免重复登记。

已有任务回执或 Wiki log 简短说明本次推进了哪个问题，或提出了什么新问题，并保留原材料与版本。没有相关推进时不另建一份记录。这些要求随下一次实际蒸馏执行；本次写入方法和实例，本身不证明未来材料已经触发过更新。

## 一次维护的最小回路

1. **定位。** 先读 `purpose.md`、总图、[TutorOS第二层经营地图](%E9%AA%A8%E6%9E%B6/TutorOS%E7%AC%AC%E4%BA%8C%E5%B1%82%E7%BB%8F%E8%90%A5%E5%9C%B0%E5%9B%BE.md) （[A](https://alidocs.dingtalk.com/i/nodes/np9zOoBVBYq1apP5IerEveo3W1DK0g6l) · [Leon](https://alidocs.dingtalk.com/i/nodes/G53mjyd80p2oeG7PcejQRwD686zbX04v)）与对应经营域，包括“FRONTIER｜接下来想弄清什么”，按新材料的实际主张选择旧页、源与决策链。输入身份、双读及预算沿原进料工艺执行；维护总图不替代这些步骤。
...
5. **更新导航。** 先核读新增页或内容变化页，维护 `wiki/.structure/knowledge-map.json` 的落点、范围、可见性及本次实读依据；不能只批量刷新指纹。依次运行 `python3 scripts/toc-map.py backlinks`、`python3 scripts/toc-map.py render`、`python3 scripts/toc-map.py check`，生成正文回链、查询入口与本地完整关系目录，并检查缺页、落点、内容变化和派生件漂移。`evidence_pages` 是兼容字段名，表示相关页面集合，可能承载决定、历史、假设或输入，不表示支持骨架所有主张。正文只有原页一份；归域与导航生成不增添业务主张或审态。
...
只有会改变目标、核心概念、模式关系、因果关系、重要适用边界或战略取舍的发现，才形成骨架议题。下层页面的普通链接、文风、重复整理、已有原文能直接裁清的日期和引用问题，由 Agent 在授权内处理。骨架页的此类修改也先提交具体差异，按 2026-09-23 人审闸门落稿。
