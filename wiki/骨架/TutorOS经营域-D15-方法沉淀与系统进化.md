...
公司级正本同向且更严：基本法第五条写明“问题不是坏事，反复发生而不沉淀才是坏事”，要求数据、流程、规则、Agent、Memory 和 Skills 持续迭代；第六条把标准化、产品化、系统化、AI 化定为建设路径。同包蜂巢篇给出对本库工作方式的约束：Memory 必须可溯源、可纠正、可过期、可撤销、可限制传播，模型输出不能未经验证直接升级为永久事实。[E10][] 同向确认不降低本页的验证标准：路径不是完成状态，沉淀也要过独立验证。
...
9 月 16 日知识库工艺有了已确认的说法：先建骨架、再蒸馏——一层骨架是业务域总地图，二层是各域“门面牌”；骨架由人类设计、AI 只出草案，理由是几百页蒸完后人改不动，骨架那十几页才是人可维护的互动面（“human stay on top，人类要站住那个首席”）。当时记下的边界：TOC 侧骨架为 V1、其他部门复用未验证（9-16 时点）。知识治理一侧，个人 Wiki 加本地正本镜像已决定（9 月 7 日），公共导览与薄路由仍未建。[E09][]

**当前重点：**

挑一个已有经验，连着看“适用条件、独立使用、后续结果、额外代价与失效时怎么退”。把成功样本与失败、异议、低课量和退出者的经历一并保留，避免只把顺利案例写成可到处复制的标准答案。

AI 化也有已记录的制衡线索：9 月 2 日周会口述，PSO 环节快速切 AI 后结果下降约 50%–60%。口述缺基线与日期口径，按口述标注，不作统计结论；它提醒 AI 化替换要带观测与回退，而不是给四化本身翻案。[E11][]2026-09-22 数仓观测注记（不改上方判断）：PSO **面试通过率**口径下方向相反——6 月 22.3% → 7 月 58.4% → 9 月 68.9%，且 7 月状态构成剧变（Failed 月量 7,601→1,961、Pre-Assigned 暴增至 5,632），前后不可直接比；后续对账（2026-09-22）：Launch 序列显示 AI 流程 2025-12 无意切换初期 Launch 月量 −57%（人工期均 1,653→12-1 月均 710，正落口述区间），2-5 月已恢复并反超——口述大概率指此事件且所述属实；7 月 Pre-Assigned 剧变为另一次独立变更。见 [PSO通过率口述对账验证](../../sources/supplement/数据验证-五洞Phase2/HQ-D15-01-PSO通过率口述对账验证.md)（数据验证补证，已审 2026-09-22；本地资料，钉钉／GitHub未提供）。
...
三条预先记下的自我约束：① 降级触发线——若持续举不出一例“第二人独立用后把工作做成”的完整经历，本假设降级为待检验的设计偏好，不再作方法推广依据；② 「对的时刻」单列——它是假设中最弱的一环；入口方便但时机不对导致没用起来时，应记「时机未成立」，不当成假设整体被证伪，也不假装时机无关；③ 反向信号——若“复用价值”的写法越来越满、而第二人独立用成的案例不增长，应给宣传降温而不是加码。
...
| 2026-09-16，AI 集训第一课（Leon 主讲，当日入库） | 知识库建库工艺定为骨架先行：一层总地图、二层门面牌，人类设计骨架、AI 出草案 | 骨架 V1、复用未验证；工艺确认不等于其他部门已采用。[E09][] |
| 9 月 18 日 [E09][] [E10][] [E11][] | 撞审修订：补接 9-16 骨架先行与知识治理决定、挂接公司级四化与 Memory 约束、按口述标注补 PSO 切 AI 反例，并为 H01 预记降级触发线与反向信号 | 修订条目经 Leon 逐项拍板（本地过审进度账）；页面整体综合审态不变 |
...
[E09]: ../queries/D15-认知依据与演化.md#e09
[E10]: ../queries/D15-认知依据与演化.md#e10
[E11]: ../queries/D15-认知依据与演化.md#e11
