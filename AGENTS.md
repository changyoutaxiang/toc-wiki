...
> 本文件是 GitHub 出口（只读投影）的固定读法入口，面向被授权访问本仓库的 Agent。
> 正本在 Leon 本地的独立 TOC LLMwiki 目录；本出口不构成维护或执行授权。
> 引用规则一律用「文件 + 节标题」，不用行号。
...
4. 每条结论应能追溯到 source；source 不在本出口内时按「三、本地依赖」降置信度。
...
- 标「本地资料，钉钉／GitHub未提供」的链接指向不在本出口内的本地文件：可用性受限，据此得出的结论置信度降档，不得当作已验证证据。
- `sources/raw/`（218 份精选原料）与 `sources/外教战役/`（历史 277 份）已在本出口内，可直接读。
- 断链或指向不存在文件的引用 = 证据缺口，如实报告，不推断内容。

## 四、边界

- 本仓库是只读投影：读取不获得维护、写入、发布或执行授权。
- 已纠正的历史说法不能复活；未决问题不自动裁决。
- 查询只返回答案 + 依据 + 适用边界。

## 五、发布质量门禁

- 出口发布前运行一致性 lint：断链、未标注本地依赖、假「未提供」标注、supersedes 双向一致、必填 frontmatter、页级审态矛盾（棘轮）。
- lint 报告存正本库 `_ops/export-lint-report.json`，本出口不携带；有 FAIL 即阻止发布。
